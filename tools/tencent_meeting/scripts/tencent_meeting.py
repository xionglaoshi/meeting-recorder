...
def _load_env_file() -> None:
    """从 `~/.codex/.env` 兜底读取凭据（setdefault：不覆盖进程内已有变量）。

    2026-09-19 修 bug：此前 `TENCENT_MEETING_TOKEN` 只从 `os.environ` 取，完全依赖
    宿主注入子进程；宿主若清洗 `*TOKEN*` 形状的变量（dsh 运行时会），本脚本必然报
    「环境变量 TENCENT_MEETING_TOKEN 未设置」。惯例同 `~/.codex/scripts/wecom_fetch.py`。
    """
